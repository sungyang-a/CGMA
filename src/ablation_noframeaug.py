#-*- coding: utf-8 -*-
"""Component ablations, including removal of frame-level augmentation (Table 5).

--ablate full        : full configuration (proxy fusion + completeness gating).
--ablate no_proxy    : final CGMA (gating, no proxy in the prediction path).
--ablate no_comp     : fixed 0.5/0.5 mixing instead of adaptive gating.
--ablate no_frameaug : on top of no_proxy, remove frame-level augmentation
                       (training sees only intact and whole-modality missingness).
Evaluates the visual frame-level curve and intact / drop-video / drop-audio.
...
def main():
    ...
    ap.add_argument('--ablate', choices=['full', 'no_proxy', 'no_comp', 'no_frameaug'], required=True)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for V, A, mv, ma, y in dls['train']:
            ...
            elif r < 0.66 and args.ablate != 'no_frameaug':   # no_frameaug: keep this branch intact (isolate frame-level augmentation)
    ...
    if args.dump_probs:
        outp = os.path.join("./outputs", f"abl_{args.ablate}_s{args.seed}.npz")
