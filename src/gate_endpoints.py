#-*- coding: utf-8 -*-
"""Endpoint gate values under intact / drop-video / drop-audio (Table 6).

Trained with the same protocol as cgma.py --ablate no_proxy; reports the mean
visual and acoustic gate outputs at the three modality-level endpoints, in
addition to the standard F1 numbers.
...
def main():
    ...
    ap.add_argument('--ablate', choices=['full', 'no_proxy', 'no_comp'], default='no_proxy')
    ...
    for mode in ['full', 'drop_v', 'drop_a']:
        ...
        print(f">>> [w|seed{args.seed}|{mode:<6}]  w_video={wv:.3f}  w_audio={wa:.3f}", flush=True)
